...
def alpha_beta(position, depth, alpha, beta, max_player, forced_caputure):
    if depth == 0 or position.get_game_end():
        return position.evaluate_state()
    if max_player: #Maximizing player so that alpha keeps getting updated increasingly higher values, that come from eval
        max_evaluation = -inf
        ...
        #Beta keeps getting updated to get increasingly lower values, that come from eval.
        min_evaluation = inf
        for child in position.get_next_moves(forced_caputure):
            eval = alpha_beta(child, depth - 1, alpha,
...
def main():
    ...
    while True:
        ...
        print(" AI AGENT ABOUT TO MAKE A DECISION !.....................")
        t1 = time() # Calculating the start time required by the AI agent using alpha-beta prunning to make a decision
        num_figures = position.count_pieces()
        if num_figures[0] + num_figures[1] > 6:
            ...
            position = max(position.get_next_moves())
        t2 = time() # Calculating the end time required by the AI agent using alpha-beta prunning to make a decision
        time_previous_move = t2 - t1
        differences = position.find_move_played(previous_table)
        print("The time it took AI agent to predict possible next move is {} seconds".format(time_previous_move))
        print_table(position.get_table(), differences)
        print("AI Agent played a move displayed on the table above.\n\n")
